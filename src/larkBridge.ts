...
export class LarkBridge {
  ...
  async handleMessage(message: LarkIncomingMessage): Promise<void> {
    ...
    if (state.processedEventIds.includes(message.eventId)) return;
    ...
    if (!this.canCreateTaskFromChat(message.chatId)) {
      await this.client.sendText(message.chatId, '这个聊天没有绑定 task，也不在允许创建 task 的控制群里。');
      return;
    }
    ...
    await this.handleControlChatMessage(message, pendingProposal);
  }
  ...
  async watchTaskStatuses(): Promise<void> {
    ...
    for (const [chatId, binding] of Object.entries(state.bindingsByChatId)) {
      ...
      if (state.notifiedStatusByTaskId[binding.taskId] === taskState.status) continue;
      await this.sendOutbound(chatId, renderStateNotification(taskState));
  ...
  private async handlePairing(message: LarkIncomingMessage): Promise<boolean> {
    ...
    if (match[1] !== state.pairingCode) {
      await this.client.sendText(message.chatId, '配对码不对。请看本地 Manager bridge 终端里打印的 pairing code。');
      return true;
    }

    await this.saveState(addPairedOpenId(state, message.senderOpenId));
    await this.client.sendText(message.chatId, `配对成功。我已经认识你了：${message.senderOpenId}`);
    return true;
  }
  ...
  private async handleTaskChatMessage(message: LarkIncomingMessage, taskId: string): Promise<void> {
    ...
    if (running) {
      await this.client.sendText(message.chatId, `这个 task 正在运行：${running.label}。请等这一步结束后再发新的状态变更。`);
      return;
    }

    await this.client.sendText(message.chatId, turn.startedMessage.text);
  ...
  private async handleUnboundCommand(
    ...
    if (command === 'help') {
      await this.client.sendText(chatId, renderControlHelp(pendingProposal));
      return;
    }
    if (command === 'stop') {
      await this.client.sendText(chatId, NO_TASK_TO_STOP_MESSAGE);
      return;
    }
    if (pendingProposal) {
      await this.client.sendText(chatId, renderControlStatus(command, pendingProposal));
      return;
    }
  ...
  private async handleControlChatMessage(
    ...
    const turn = await this.conversation.routeControlMessage(message.text, pendingProposal);
    if (turn.kind === 'reply') {
  ...
  private async confirmPendingProposal(
    ...
    if (!pendingProposal) {
      await this.client.sendText(message.chatId, noPendingProposalMessage('There is no pending proposal to create.'));
      return;
    }
  ...
  private async editPendingProposal(
    ...
    if (!pendingProposal) {
      await this.client.sendText(message.chatId, noPendingProposalMessage('There is no pending proposal to edit.'));
      return;
    }

    const turn = await this.conversation.reviseControlProposal(instruction, pendingProposal);
    if (turn.kind === 'reply') {
      await this.sendOutbound(message.chatId, turn.message);
      return;
    }
    ...
    const state = await this.loadState();
    state.pendingProposalsByChatId[message.chatId] = makePendingProposal(turn.proposal, message, pendingProposal.originalMessage);
    await this.saveState(state);
    await this.sendOutbound(message.chatId, turn.message);
  }
  ...
  private async cancelPendingProposal(
    ...
    if (!pendingProposal) {
      await this.client.sendText(chatId, noPendingProposalMessage('There is no pending proposal to cancel.'));
      return;
    }
    ...
    await this.saveState(state);
    await this.client.sendText(chatId, 'Canceled the pending task proposal. No task was created.');
  }
  ...
  private async createTaskFromMessage(
    ...
    try {
      ...
      if (!allowFallbackToCurrentChat) {
        await this.client.sendText(message.chatId, [
    ...
    await this.client.sendText(taskChatId, [
  ...
  private async runBackgroundJob(
    ...
    const state = await this.loadState();
    state.runningJobsByTaskId[taskId] = { taskId, label, startedAt: new Date().toISOString() };
    await this.saveState(state);
  ...
  private async notifyJobDone(chatId: string, taskId: string, result: WorkflowResult): Promise<void> {
    ...
    await this.saveState(state);
    await this.sendOutbound(chatId, renderWorkflowResult(result));
  }
  ...
  private async notifyJobError(chatId: string, taskId: string, error: unknown): Promise<void> {
    ...
    delete state.runningJobsByTaskId[taskId];
    await this.saveState(state);
    await this.client.sendText(chatId, `后台任务失败：${error instanceof Error ? error.message : String(error)}`);
  }
  ...
  private async handleProjectCommand(
    ...
    if (command.kind === 'list') {
      await this.client.sendText(message.chatId, renderProjectList(this.config, state.activeProjectIdByChatId[message.chatId]));
      return;
    }
    ...
    if (command.kind === 'none') {
      ...
      await this.saveState(state);
      await this.client.sendText(message.chatId, `Project selection cleared. New tasks will use default project: ${getDefaultProjectId(this.config)}`);
      return;
    }
    ...
    await this.saveState(state);
    await this.client.sendText(message.chatId, `Active project set to ${project.name} (${project.id}).`);
  }

  private async sendOutbound(chatId: string, outbound: OutboundMessage): Promise<void> {
    await this.client.sendText(chatId, outbound.text);
...
function parseProposalReply(text: string): ProposalReply | undefined {
  ...
  if (normalized === 'create task' || normalized === 'yes create' || normalized === 'confirm') {
    return { kind: 'confirm' };
  }
