...
export class WorkflowService {
  ...
  async planTask(taskIdOrLatest: string): Promise<WorkflowResult> {
    ...
    if (!state.briefConfirmed) {
      ...
      state = {
        ...
        pendingUserPrompt: 'Review the brief: approve A to start planning, revise C: <correction> to refine, reject B to stop.',
  ...
  async implementApproved(taskIdOrLatest: string): Promise<WorkflowResult> {
    ...
    state = { ...state, status: 'implementing', updatedAt: new Date().toISOString() };
    await this.store.saveState(state);
    ...
    state = await this.store.writeArtifact(state, 'implementation-log', implementation.markdown);
    ...
    state = await this.store.writeArtifact(state, 'test-build-log', renderVerificationLog(verification));
    state = { ...state, status: 'implemented', updatedAt: new Date().toISOString() };
    await this.store.saveState(state);

    return this.finalReview(state.taskId, preGit, postGit);
  }

  async finalReview(taskIdOrLatest: string, preGit?: GitSnapshot, postGit?: GitSnapshot): Promise<WorkflowResult> {
    let state = await this.store.loadState(taskIdOrLatest);
    ...
    if (route.route === 'complete') {
      const before = preGit ?? {
        statusShort: await this.store.readArtifact(state, 'git-pre-status').catch(() => ''),
        diff: await this.store.readArtifact(state, 'git-pre-diff').catch(() => ''),
      };
      const after = postGit ?? {
        statusShort: await this.store.readArtifact(state, 'git-post-status').catch(() => ''),
        diff: await this.store.readArtifact(state, 'git-post-diff').catch(() => ''),
      };
      const report = await this.renderFinalReport(state, route.reason, before, after);
      state = await this.store.writeArtifact(state, 'final-report', report);
      state = { ...state, status: 'completed', updatedAt: new Date().toISOString() };
      await this.store.saveState(state);
      return { state, message: 'Task completed and final report generated.' };
    }
...
function readPlanMetadata(revisedPlan: string): { verificationCommands: string[] } {
  const match = revisedPlan.match(/<!-- manager-plan-metadata\s*([\s\S]*?)\s*manager-plan-metadata -->/);
  if (!match?.[1]) return { verificationCommands: [] };
  try {
    const payload = JSON.parse(match[1]) as { verificationCommands?: unknown };
    return {
