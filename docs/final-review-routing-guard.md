...
Final Review routing is a Manager-owned decision point. The LLM can advise whether a failed final review should be completed, sent back to implementation, sent back to planning, or surfaced to the user, but it must not be the only safety layer.
...
4. Manager paused with an internal formatting error instead of giving the user an actionable choice.

From the user's perspective, "Manager" and "Advisor" are both Elon Ma. The code should not expose internal role-format errors as normal workflow guidance.

## Rule

Manager keeps deterministic guardrails after Final Review:
...
- If a user decision is needed but the advisor omits structured A/B/C/D options, Manager generates safe fallback options.
...
Manager still limits automatic Final Review follow-up loops. After the automatic follow-up budget is used, repeated implementation blockers pause with explicit options:
...
This cap is Manager-generated. It should not rely on the routing advisor to produce the options.
...
Then, when Manager says it has recorded the direction and is ready, reply:
