# 分发化：preflight + package scripts + Windows 启动 + docs 衔接

## Task Info

| Field | Value |
|---|---|
| Task ID | 20260525-223603-preflight-package-scripts-windows-docs |
| Title | 分发化：preflight + package scripts + Windows 启动 + docs 衔接 |
| Category | Other |
| Status | created |
...
`docs/agent-setup-guide.md` 里如果提到 AI agent 可以 inspect `.[env.local](http://env.local/)`，请改成更安全的表述：
- agent 可以检查 `.[env.local](http://env.local/)` 是否存在。
- agent 可以检查需要的 env var name 是否存在且非空。
- agent 不应该读取、复述、打印、总结、保存或提交真实 secret 值。
- 如果必须写入 secret，应该让用户自己在本地文件中填写，或者只在用户明确授权时写入 `.[env.local](http://env.local/)`，并且不能在输出里回显完整 secret。
...
- 检查内容：
  ...
  - `.[env.local](http://env.local/)` 是否存在。
  - config 能否 load。
  - `profiles.*.apiKeyEnv` 引用的 env var 是否存在于 process env 或 `.[env.local](http://env.local/)`。
...
How to fix:
1. Open .[env.local](http://env.local/)
...
Query usage from this workflow repo: `npm run assistant -- usage --task 20260525-223603-preflight-package-scripts-windows-docs --by role`
